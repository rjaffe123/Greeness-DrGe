...
# tt_log_2005 <- tt_log %>% filter(CDNAME == "Toronto")
tt_log_2019 <- read.csv("/home/rjaffe/tt_log_2019.csv")
shp.toronto <- shapefile("/home/rjaffe/toronto/Neighbourhoods.shp")
lonlat <- cbind(tt_log_2019$longitude, tt_log_2019$latitude)
sp.case <- SpatialPoints(lonlat,CRS("+proj=longlat +datum=WGS84"))
newcrs <- proj4string(shp.toronto)
sp.case <- spTransform(sp.case,	CRS(newcrs))

## Parks
#driving
tt_2019_park <- tt_log_2019 %>% filter(year == 2019 & CDNAME == "Toronto" & mode == "driving" & green_type == "park")
lonlat <- cbind(tt_2019_park$longitude, tt_2019_park$latitude)
sp.case <- SpatialPoints(lonlat,CRS("+proj=longlat +datum=WGS84"))
newcrs <- proj4string(shp.toronto)
sp.case<-spTransform(sp.case, CRS(newcrs))
spdf.case <- SpatialPointsDataFrame(sp.case,	data=tt_2019_park)
regions <- slot(shp.toronto, "polygons")
regions <- lapply(regions, function(x){ SpatialPolygons(list(x))})
windows <- lapply(regions, as.owin)
shp.toronto1 <-tess(tiles=windows)
w.toronto <- as.owin(shp.toronto1)
...
pdf("/home/rjaffe/toronto_park_driving.pdf", width = 10, height = 8)
plot(rho.hat, tol.args = list(levels = c(0.01, 0.05), lty = 1:2), xlab = "Easting", ylab = "Northing")
dev.off()

## Forest
#driving
tt_2019_forest <- tt_log_2019 %>% filter(year == 2019 & CDNAME == "Toronto" & mode == "driving" & green_type == "forest")
...
pdf("/home/rjaffe/toronto_forest_driving.pdf", width = 10, height = 8)
plot(rho.hat, tol.args = list(levels = c(0.01, 0.05), lty = 1:2), xlab = "Easting", ylab = "Northing")
dev.off()
...
##Walking
#Park
tt_2019_park <- tt_log_2019 %>% filter(year == 2019 & CDNAME == "Toronto" & mode == "walking" & green_type == "park")
lonlat <- cbind(tt_2019_park$longitude, tt_2019_park$latitude)
sp.case <- SpatialPoints(lonlat,CRS("+proj=longlat +datum=WGS84"))
newcrs <- proj4string(shp.toronto)
sp.case<-spTransform(sp.case, CRS(newcrs))
...
pdf("/home/rjaffe/toronto_park_walking.pdf", width = 10, height = 8)
plot(rho.hat, tol.args = list(levels = c(0.01, 0.05), lty = 1:2), xlab = "Easting", ylab = "Northing")
dev.off()


## Forest
tt_2019_forest <- tt_log_2019 %>% filter(year == 2019 & CDNAME == "Toronto" & mode == "walking" & green_type == "forest")
lonlat <- cbind(tt_2019_forest$longitude, tt_2019_forest$latitude)
sp.case <- SpatialPoints(lonlat,CRS("+proj=longlat +datum=WGS84"))
newcrs <- proj4string(shp.toronto)
sp.case<-spTransform(sp.case, CRS(newcrs))
...
pdf("/home/rjaffe/toronto_forest_walking.pdf", width = 10, height = 8)
plot(rho.hat, tol.args = list(levels = c(0.01, 0.05), lty = 1:2), xlab = "Easting", ylab = "Northing")
dev.off()
