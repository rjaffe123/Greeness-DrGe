...
```{r 2019}
merged_ct_ottawa_pw  <- merged_ct_ottawa %>% filter(year == 2019, mode=="driving", green_type == "park")
merged_ct_ottawa_pw <- merged_ct_ottawa_pw %>% mutate(travel_time = replace_na(travel_time, 120))
...
pdf("ot_driving_park_2019.pdf", width = 10, height = 8)
spplot(combined, "ave_tt", main = list(label="Averge Travel Time to Drive to a park (in Minutes) per Census Tract - 2019", cex = 1), col.regions = my.palette, cuts = 6)
dev.off()
...
pdf("ot_driving_park_MWscan_2019.pdf", width = 10, height = 8)
spplot(combined, "cluster_final", col.regions = mycols)
dev.off()

summary_2019<- merged_ct_toronto_pw1 %>% group_by(cluster_final) %>% summarize(average = mean(ave_tt))

```

```{r 2015}
merged_ct_ottawa_pw  <- merged_ct_ottawa %>% filter(year == 2015, mode=="driving", green_type == "park")
merged_ct_ottawa_pw <- merged_ct_ottawa_pw %>% mutate(travel_time = replace_na(travel_time, 120))
merged_ct_ottawa_pw <- merged_ct_ottawa_pw %>% group_by(CTNAME) %>% mutate(ave_tt = mean(travel_time, na.rm=TRUE))
...
pdf("ot_driving_park_2015.pdf", width = 10, height = 8)
spplot(combined, "ave_tt", main = list(label="Averge Travel Time to Drive to a park (in Minutes) per Census Tract - 2015", cex = 1), col.regions = my.palette, cuts = 6)
dev.off()
...
pdf("ot_driving_park_MWscan_2015.pdf", width = 10, height = 8)
spplot(combined, "cluster_final", col.regions = mycols)
dev.off()

summary_2015<- merged_ct_toronto_pw1 %>% group_by(cluster_final) %>% summarize(average = mean(ave_tt))
```

```{r 2010}
merged_ct_ottawa_pw  <- merged_ct_ottawa %>% filter(year == 2010, mode=="driving", green_type == "park")
merged_ct_ottawa_pw <- merged_ct_ottawa_pw %>% mutate(travel_time = replace_na(travel_time, 120))
merged_ct_ottawa_pw <- merged_ct_ottawa_pw %>% group_by(CTNAME) %>% mutate(ave_tt = mean(travel_time, na.rm=TRUE))
...
pdf("ot_driving_park_2010.pdf", width = 10, height = 8)
spplot(combined, "ave_tt", main = list(label="Averge Travel Time to Drive to a park (in Minutes) per Census Tract - 2010", cex = 1), col.regions = my.palette, cuts = 6)
dev.off()
...
pdf("ot_driving_park_MWscan_2010.pdf", width = 10, height = 8)
spplot(combined, "cluster_final", col.regions = mycols)
dev.off()
summary_2010<- merged_ct_toronto_pw1 %>% group_by(cluster_final) %>% summarize(average = mean(ave_tt))
```

```{r 2005}
merged_ct_ottawa_pw  <- merged_ct_ottawa %>% filter(year == 2005, mode=="driving", green_type == "park")
merged_ct_ottawa_pw <- merged_ct_ottawa_pw %>% mutate(travel_time = replace_na(travel_time, 120))
merged_ct_ottawa_pw <- merged_ct_ottawa_pw %>% group_by(CTNAME) %>% mutate(ave_tt = mean(travel_time, na.rm=TRUE))
...
pdf("ot_driving_park_2005.pdf", width = 10, height = 8)
spplot(combined, "ave_tt", main = list(label="Averge Travel Time to Drive to a park (in Minutes) per Census Tract - 2005", cex = 1), col.regions = my.palette, cuts = 6)
dev.off()
...
pdf("ot_driving_park_MWscan_2005.pdf", width = 10, height = 8)
spplot(combined, "cluster_final", col.regions = mycols)
dev.off()
summary_2005<- merged_ct_toronto_pw1 %>% group_by(cluster_final) %>% summarize(average = mean(ave_tt))
```
...
long_lat <- merged_ct_ottawa %>% filter(mode=="driving", green_type == "park")
...
pdf("ot_driving_park_MWscan_allyears.pdf", width = 10, height = 8)
